...
it('feed shows articles by followers', async ()=>{
  ...
  const user0 = await sequelize.models.User.findOne({where: {email: 'user0@mail.com'}})
  ...
  assert.strictEqual(rows[0].title, 'My title 4')
  assert.strictEqual(rows[1].title, 'My title 3')
  assert.strictEqual(rows[2].title, 'My title 2')
  assert.strictEqual(rows[3].title, 'My title 0')
