...
it(`api: requests`, async () => {
  await testApp(async (test) => {
    ...
    // Make a request to /api/users without referrer
    config.devIp = '123.123.123.1'
    const user0 = await test.createUserApi(0)
    assertRows(
      await Request.findAll({ order: [['createdAt', 'ASC']] }),
      [
        { ip: '123.123.123.1', path: '/api/users', referrer: null },
      ]
    )

    // Make a request to /api/articles?id=user0 without referrer from same IP.
    // It does not get tracked again, only once per IP without referrer.
    config.devIp = '123.123.123.1'
    ;({data, status} = await test.webApi.article('user0'))
    assertRows(
      await Request.findAll({ order: [['createdAt', 'ASC']] }),
    ...
    // Same but now with a different IP. It gets tracked.
    ...
    assertRows(
      ...
      [
        { ip: '123.123.123.1', path: '/api/users', referrer: null },
        { ip: '123.123.123.2', path: '/api/articles?id=user0', referrer: null },
      ]
    )

    // Back to original IP with referrer it does get tracked.
    config.devIp = '123.123.123.1'
