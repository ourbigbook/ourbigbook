...
it('Article descending topic ordering index avoids sorting the last article page', async function() {
  const sequelize = this.test.sequelize
  const qi = sequelize.getQueryInterface()
  const migration = require('./migrations/21000101000046-article-add-topic-desc-order-index')
  const indexName = 'article_list_topic_id_desc_created_at_id'
  const indexNames = async () => (await qi.showIndex('Article')).map(index => index.name).sort()
  const originalIndexes = await indexNames()
  assert(originalIndexes.includes(indexName))
  ...
  async function assertOrderedIndexScan() {
    ...
    await sequelize.transaction(async transaction => {
      ...
      // selector query used by /-/articles?page=5965&sort=id-desc.
      await sequelize.query('SET LOCAL enable_seqscan = off', { transaction })
      const [rows] = await sequelize.query(`EXPLAIN (FORMAT JSON)
        SELECT "id" FROM "Article" WHERE "list" = true
        ORDER BY "topicId" DESC NULLS LAST, "createdAt" DESC NULLS LAST, "id" DESC
        LIMIT 20 OFFSET 2`, { transaction })
      const plan = rows[0]['QUERY PLAN'][0].Plan
      const nodes = []
      const visit = node => {
...
it('Article creation ordering index avoids sorting the last HTML article page', async function() {
  const sequelize = this.test.sequelize
  const qi = sequelize.getQueryInterface()
  const migration = require('./migrations/21000101000051-article-add-list-created-order-index')
  const indexName = 'article_list_created_at_id'
  const indexNames = async () => (await qi.showIndex('Article')).map(index => index.name).sort()
  const originalIndexes = await indexNames()
  assert(originalIndexes.includes(indexName))
  ...
  async function assertOrderedIndexScan() {
    ...
    await sequelize.transaction(async transaction => {
      // The small fixture would normally favour a sequential scan. This is the
      // selector used by /-/articles?page=5965&sort=created.
      await sequelize.query('SET LOCAL enable_seqscan = off', { transaction })
      const [rows] = await sequelize.query(`EXPLAIN (FORMAT JSON)
        SELECT "id" FROM "Article" WHERE "list" = true
...
it('Article score ordering index avoids sorting the last HTML article page', async function() {
  const sequelize = this.test.sequelize
  const qi = sequelize.getQueryInterface()
  const migration = require('./migrations/21000101000052-article-add-list-score-order-index')
  const indexName = 'article_list_score_created_at_id'
  const indexNames = async () => (await qi.showIndex('Article')).map(index => index.name).sort()
  const originalIndexes = await indexNames()
  assert(originalIndexes.includes(indexName))
  ...
  async function assertOrderedIndexScan() {
    ...
    await sequelize.transaction(async transaction => {
      // The small fixture would normally favour a sequential scan. This is the
      // selector used by /-/articles?page=5965&sort=score.
      await sequelize.query('SET LOCAL enable_seqscan = off', { transaction })
      const [rows] = await sequelize.query(`EXPLAIN (FORMAT JSON)
        SELECT "id" FROM "Article" WHERE "list" = true
        ORDER BY "score" DESC NULLS LAST, "createdAt" DESC NULLS LAST, "id" DESC
        LIMIT 20 OFFSET 2`, { transaction })
      const plan = rows[0]['QUERY PLAN'][0].Plan
      const nodes = []
      const visit = node => {
...
it('Topic ID ordering index avoids sorting the last topic page', async function() {
  const sequelize = this.test.sequelize
  const qi = sequelize.getQueryInterface()
  const migration = require('./migrations/21000101000047-topic-add-topic-order-index')
  const indexName = 'topic_topic_id_created_at'
  ...
  async function assertOrderedIndexScan() {
    ...
    await sequelize.transaction(async transaction => {
      // The small fixture would normally favour a sequential scan. This is the
      // selector query used by /api/topics?limit=20&offset=118660&sort=id.
      await sequelize.query('SET LOCAL enable_seqscan = off', { transaction })
      const [rows] = await sequelize.query(`EXPLAIN (FORMAT JSON)
        SELECT "id" FROM "Topic"
        ORDER BY "topicId" DESC, "createdAt" DESC
        LIMIT 20 OFFSET 2`, { transaction })
      const plan = rows[0]['QUERY PLAN'][0].Plan
      const nodes = []
      const visit = node => {
...
it('Article author topic ordering index avoids filtering and sorting the author page', async function() {
  const sequelize = this.test.sequelize
  const qi = sequelize.getQueryInterface()
  const migration = require('./migrations/21000101000044-article-add-author-topic-order-index')
  const indexName = 'article_author_id_topic_id_created_at_id'
  const indexNames = async () => (await qi.showIndex('Article')).map(index => index.name).sort()
  const originalIndexes = await indexNames()
  assert(originalIndexes.includes(indexName))
  ...
  async function assertOrderedIndexScan() {
    if (sequelize.options.dialect !== 'postgres') return
    await sequelize.transaction(async transaction => {
      // The small test fixture would normally favour a sequential scan.
      await sequelize.query('SET LOCAL enable_seqscan = off', { transaction })
      const [rows] = await sequelize.query(`EXPLAIN (FORMAT JSON)
        SELECT "id" FROM "Article" WHERE "authorId" = :authorId
        ORDER BY "topicId" DESC, "createdAt" DESC NULLS LAST, "id" DESC
...
it('Article author score ordering index avoids filtering and sorting deep API pages', async function() {
  const sequelize = this.test.sequelize
  const qi = sequelize.getQueryInterface()
  const migration = require('./migrations/21000101000048-article-add-author-score-order-index')
  const indexName = 'article_author_id_score_created_at_id'
  const indexNames = async () => (await qi.showIndex('Article')).map(index => index.name).sort()
  const originalIndexes = await indexNames()
  assert(originalIndexes.includes(indexName))
  ...
  async function assertOrderedIndexScan() {
    ...
    await sequelize.transaction(async transaction => {
      // The small fixture would normally favour a sequential scan. This is the
      // selector used by the deep author sort=score benchmark request.
      await sequelize.query('SET LOCAL enable_seqscan = off', { transaction })
      const [rows] = await sequelize.query(`EXPLAIN (FORMAT JSON)
        SELECT "id" FROM "Article" WHERE "authorId" = :authorId
        ORDER BY "score" DESC, "createdAt" DESC NULLS LAST, "id" DESC
        LIMIT 20 OFFSET 2`, { replacements: { authorId: user.id }, transaction })
      const plan = rows[0]['QUERY PLAN'][0].Plan
      const nodes = []
      const visit = node => {
...
it('Article topic ordering index avoids sorting deep global API pages', async function() {
  const sequelize = this.test.sequelize
  const qi = sequelize.getQueryInterface()
  const migration = require('./migrations/21000101000049-article-add-topic-api-order-index')
  const indexName = 'article_topic_id_created_at_id'
  const indexNames = async () => (await qi.showIndex('Article')).map(index => index.name).sort()
  const originalIndexes = await indexNames()
  assert(originalIndexes.includes(indexName))
  ...
  async function assertOrderedIndexScan() {
    ...
    await sequelize.transaction(async transaction => {
      // The small fixture would normally favour a sequential scan. This is the
      // selector used by the deep global API sort=id benchmark request.
      await sequelize.query('SET LOCAL enable_seqscan = off', { transaction })
      const [rows] = await sequelize.query(`EXPLAIN (FORMAT JSON)
...
it('File author path ordering index avoids sorting article hash manifests', async function() {
  const sequelize = this.test.sequelize
  const qi = sequelize.getQueryInterface()
  const migration = require('./migrations/21000101000045-file-add-author-path-index')
  const indexName = 'file_author_id_path'
  const indexNames = async () => (await qi.showIndex('File')).map(index => index.name).sort()
  const originalIndexes = await indexNames()
  assert(originalIndexes.includes(indexName))
  ...
  async function assertOrderedIndexScan() {
    ...
    await sequelize.transaction(async transaction => {
      await sequelize.query('SET LOCAL enable_seqscan = off', { transaction })
      const [rows] = await sequelize.query(`EXPLAIN (FORMAT JSON)
        SELECT "id" FROM "File" WHERE "authorId" = :authorId
        ORDER BY "path" ASC LIMIT 20 OFFSET 2`, {
        replacements: { authorId: user.id }, transaction,
      })
      const plan = rows[0]['QUERY PLAN'][0].Plan
      const nodes = []
      const visit = node => {
